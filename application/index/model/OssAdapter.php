...
class OssAdapter extends Model{
	...
	public function getThumb(){
		if(!$this->policyModel['bucket_private']){
			...
			$baseUrl = $this->policyModel["url"].$this->fileModel["pre_name"];
			try{
				$file_format = Commonfun::file_format($this->fileModel["pre_name"]);
			    if ($file_format=='video'){
                    $signedUrl = $ossClient->signUrl($this->policyModel["bucketname"], $this->fileModel["pre_name"], Option::getValue("timeout"),'GET', array("x-oss-process" => 'video/snapshot,t_1000,f_jpg,h_0,w_0,m_fast,ar_auto'));
                }
			    elseif($file_format == 'img'){
                    $signedUrl = $ossClient->signUrl($this->policyModel["bucketname"], $this->fileModel["pre_name"], Option::getValue("timeout"),'GET', array("x-oss-process" => 'image/resize,m_fill,h_100,w_100'));
                }
			    else{
			    	$signedUrl = '/static/img/logo_s.png';
				}
