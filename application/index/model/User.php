...
class User extends Model{
     ...
     */
	static function register($userEmail,$userPass,$captchaCode,$openid){
		...
		$is_exist_mail = Db::name('users')->where('user_email',$userName)->find();
		if($is_exist_mail !=null && !$openid){
		...
		if($is_exist_mail==null && Db::name('users')->insert($sqlData)){
			...
			$db = db('users');
			$db -> where('user_email',$is_exist_mail['user_email'])->where('user_pass',md5(config('salt').$passWord)) -> update(['openid' => $openid]);
           # $db -> where('user_email',$is_exist_mail['user_email']) -> update(['openid' => $openid]);
            return [true,"绑定成功"];
		}
	}
	...
	static function login($userEmail,$userPass,$captchaCode){
        ...
        cookie('openid',Db::name('users')->where('user_email',$userEmail)->value('openid'));
